...
class NotarScraper(BaseRealtorScraper):
    """Scraper for Notar listings"""
    ...
    def scrape_listings(self, max_results=50, download_images=True):
        ...
        try:
            ...
            # Find listing links - Notar uses /kopa-bostad/objekt/[ID] pattern
            ...
            for link in listing_links:
                href = link.get('href', '')
                match = re.search(r'/objekt/([A-Z0-9]+)', href)
                if match:
    ...
    def _scrape_listing_details(self, driver, url, listing_id):
        """Scrape full details from a Notar listing page"""
        try:
            driver.get(url)
            time.sleep(3)
            ...
            if len(listing.get('image_urls', [])) < 5:
                img_elements = soup.select('img[src*="images"], img[src*="cdn"], img[src*="media"], img[data-src]')
